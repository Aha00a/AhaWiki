...
object WikiPermission {
  def apply()(implicit request: Request[Any], syncCacheApi: SyncCacheApi, database:Database): WikiPermission = new WikiPermission()
}

class WikiPermission(implicit request: Request[Any], syncCacheApi: SyncCacheApi, database:Database) {
