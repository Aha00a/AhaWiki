...
object MacroSimilarPages extends TraitMacro with Logging {
  ...
  def getMarkupSimilarPages(name: String)(implicit wikiContext: ContextWikiPage): String = {
    wikiContext.database.withConnection { implicit connection =>
      ...
      val cosineSimilarities: immutable.Seq[CalculatedCosineSimilarity] = CalculatedCosineSimilarity
        .select(name)
        .view
      ...
      if (cosineSimilarities.isEmpty) {
        ...
        val seqHighScoredTerm: Seq[HighScoredTerm] = PageLogic.selectHighScoredTerm(name, similarPageNames)
        val mapHighScoredTerm: Map[String, Seq[HighScoredTerm]] = seqHighScoredTerm.groupBy(_.name)
        val highScoredTerms: Map[String, String] = mapHighScoredTerm.view.mapValues(_.take(10).map(h => s"${h.term}(${h.frequency1}:${h.frequency2})").mkString(", ")).toMap
