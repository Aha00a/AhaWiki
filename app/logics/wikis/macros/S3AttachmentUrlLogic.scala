  ...
  def generatePresignedUrl(objectKey: String)(implicit wikiContext: ContextWikiPage): Either[String, String] = {
    Try {
      val credentials = new BasicAWSCredentials(
        wikiContext.applicationConf.AhaWiki.aws.AWS_ACCESS_KEY_ID(),
        wikiContext.applicationConf.AhaWiki.aws.AWS_SECRET_ACCESS_KEY(),
      )
      val amazonS3: AmazonS3 = AmazonS3ClientBuilder.standard
        .withCredentials(new AWSStaticCredentialsProvider(credentials))
        .withRegion(wikiContext.applicationConf.AhaWiki.aws.AWS_REGION())
        .build()

      val bucket = wikiContext.applicationConf.AhaWiki.aws.s3.bucket()
