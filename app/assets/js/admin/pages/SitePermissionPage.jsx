...
import {formatPermissionAction, toPermissionActionOptions, compareValuesForSort} from "../utils.js";
...
export default function SitePermissionPage() {
    ...
    const actionOptions = useMemo(() => toPermissionActionOptions(permissionVocabulary.actions), [permissionVocabulary.actions]);
    ...
    return (
        <>
            ...
            <SimpleGrid cols={{base: 1, md: 5}} spacing="sm" mb="sm">
                <Select label="targetType" data={permissionVocabulary.targetTypes} value={form.targetType} onChange={(v) => setForm({...form, targetType: v ?? "All", target: v === "All" ? "" : form.target})}/>
                ...
                <Select label="actorType" data={permissionVocabulary.actorTypes} value={form.actorType} onChange={(v) => setForm({...form, actorType: v ?? "All", actor: v === "All" || v === "Login" ? "" : form.actor})}/>
                <TextInput label="actor" value={form.actor} onChange={(e) => setForm({...form, actor: e.currentTarget.value})} placeholder="email or @domain"/>
                <Select label="action" data={actionOptions} value={form.action} onChange={(v) => setForm({...form, action: v ?? "Read"})}/>
            </SimpleGrid>
            <Group mb="md">
            ...
            <SimpleGrid cols={{base: 1, md: 4}} spacing="sm" mb="sm">
                ...
                <Select label="action" data={actionOptions} value={diagnoseForm.action} onChange={(v) => setDiagnoseForm({...diagnoseForm, action: v ?? "Read"})}/>
