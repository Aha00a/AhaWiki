    ...
    return (
        <Card withBorder radius="md" padding="lg">
            ...
            <Table striped highlightOnHover withTableBorder withColumnBorders>
                <Table.Thead><Table.Tr><Table.Th>Seq</Table.Th><Table.Th>Name</Table.Th><Table.Th>Abbr</Table.Th><Table.Th>Main Domain</Table.Th><Table.Th>Domains</Table.Th><Table.Th>Pages</Table.Th><Table.Th>Action</Table.Th></Table.Tr></Table.Thead>
                <Table.Tbody>
                    {sites.map((site) => (
                        <Table.Tr key={site.seq}>
                            <Table.Td>{site.seq}</Table.Td><Table.Td>{site.name}</Table.Td><Table.Td>{site.abbr ?? "-"}</Table.Td><Table.Td>{site.mainDomain ?? "-"}</Table.Td><Table.Td>{(site.domains ?? []).join(", ") || "-"}</Table.Td><Table.Td>{site.pageCount ?? 0}</Table.Td>
