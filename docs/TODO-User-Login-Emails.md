...
- [ ] 4. `UserEmail` 스키마 migration을 작성한다.
...
- [ ] 6. `User` model에서 `email` 의존성을 제거한다.
...
- [ ] 7. `User.selectOrInsert`를 `UserEmail.email` 기준으로 변경한다.
...
- [ ] 9. Google OAuth 일반 로그인과 계정 연결 flow를 분리한다.
...
- [ ] 10. 권한 평가를 현재 사용자의 모든 로그인 이메일 기준으로 변경한다.
...
- [ ] 11. `User.email`을 직접 참조하는 controller/view/API/admin 코드를 제거한다.
...
- [ ] 13. 이미 다른 user에 연결된 이메일의 병합 확인 flow를 구현한다.
...
- [ ] 14. user 병합 시 모든 `User(seq)` 참조를 canonical user로 옮긴다.
  ...
  병합 후 duplicate `User` row를 삭제할지, 비활성 보존할지는 2번 정책 결정에 따른다.
