...
| [docs/ahawiki.net/Dev Logs](docs/ahawiki.net/Dev%20Logs) | Where the production log is, the three commands that turn 491MB of it into a list of what is broken, and what that list held the first time anyone ran them. Also the fact that nothing rotates it. |
| [docs/ahawiki.net/Dev Database](docs/ahawiki.net/Dev%20Database) | The database itself: what the columns mean, which timezone they are in, and why a new table has to say `COLLATE=utf8mb4_bin` itself — the database default is something else, and evolution 68 found that out in production. |
| [docs/ahawiki.net/Dev SchemaOrgVocabulary](docs/ahawiki.net/Dev%20SchemaOrgVocabulary) | Where `public/schema.org/` comes from. Not what schema.org publishes: a separate repository strips the `@` and the prefixes, which is why `SchemaOrg.scala` can read `id` and `Class` directly. How to raise the version, and what 30.1 would bring — including 231 foreign-vocabulary terms and the `owner` property the importer has been waiting for. |
...
| [test/manual/editor-hotkeys/](test/manual/editor-hotkeys/) | Presses Alt+S, Alt+R and Alt+H against the real `edit.hotkeyAlt.js` and counts what gets clicked. It exists because the dispatcher once clicked the navigation bar's hints too, and Alt+H left the editor with the text unsaved. Same shape as the one above: a browser, no server. |
 ...
 * One kind of staleness is checked for you: `test/ahawiki.docs-citations.test.mjs` fails when a wiki page cites a class, method, or file by a name that no longer exists in the source. That check reads names only, which is what a rename leaves behind. A name that is real but lives outside this repository goes in that file's `namedOutsideTheRepository` list with the reason it is not here.
 ...
 * And it checks for a move, which a name check cannot see: a cited `Owner.member` must still be a member of `Owner`. The test finds where `Owner` is declared and looks for `member` there. `Api.adminRecentChanges` passed the name check for a month after the method went to `ApiAdminReport`. Owners declared nowhere in this repository — `Json`, `Random`, schema.org classes — are skipped. A member quoted for what it once was goes in `membersQuotedAsHistory` with the reason.
...
Task documents under `docs/ahawiki.net/` are named `ToDo…` — that spelling, which is the wiki's own WikiWord naming. English Wikipedia's `Todo` is a disambiguation page and settles nothing, so the wiki's convention decides. They should be maintained as checkbox lists. Always update the relevant checkbox state immediately after finishing the work, as part of the same change — this is required, never optional. Do not end a task with completed work still marked as an open checkbox:
...
When a ToDo document is fully completed, do not keep it as a historical checklist by default:
